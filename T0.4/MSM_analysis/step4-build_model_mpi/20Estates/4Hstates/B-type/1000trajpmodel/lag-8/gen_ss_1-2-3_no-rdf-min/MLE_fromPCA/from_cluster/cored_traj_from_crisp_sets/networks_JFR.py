...
class NetworkPlot(object):
    ...
    def _draw_arrow(self, x1, y1, x2, y2, Dx, Dy, label="", width=1.0,
        ...
        tail_width = width
        head_width = max(0.25,2*width) # max(0.5, 2 * width) # JFR
    ...
    def plot_network(self,
                     ...
                     arrow_label_format='%10.2f', max_width=12, max_height=12, max_flux=None,
            ...
            for j in range(i + 1, n):
                if ( abs(self.A[i,j]) > 0 ):
                    ...
                    self._draw_arrow(self.pos[i, 0], self.pos[i, 1],
                                     ...
                                     label=str(L[i, j]),
                                     width=arrow_scale * abs(self.A[i, j]), color=color,
                ...
                if ( abs(self.A[j,i]) > 0 ):
                    ...
                    self._draw_arrow(self.pos[j, 0], self.pos[j, 1],
                                     ...
                                     label=str(L[j, i]),
                                     width=arrow_scale * abs(self.A[j, i]), color=color,
                                     arrow_curvature=arrow_curvature,
...
def plot_flux(flux, pos=None, state_sizes=None, flux_scale=1.0,
              ...
              arrow_scale=1.0, arrow_curvature=1.0, arrow_labels='weights',
              arrow_label_format='%2.e', max_width=12, max_height=12, max_flux=None,
    ...
    plot = NetworkPlot(F, pos=pos)
    if minflux > 0:
        I, J = np.where(np.abs(F) < minflux)
        F[I, J] = 0.0
    ax = plot.plot_network(state_sizes=state_sizes, state_scale=state_scale,
                           state_colors=state_colors, state_labels=state_labels,
                           arrow_scale=arrow_scale, arrow_curvature=arrow_curvature,
                           arrow_labels=arrow_labels,
                           arrow_label_format=arrow_label_format,
                           max_width=max_width, max_height=max_height, max_flux=None,
