...
# loop over different solvation shell selections within the md loop
def calc_wghtd_CN_multss( mdtraj, (sel, pairs, rcut, r, weights) ):
    # get the number of solvation shells, assume for now sel, pairs and r is the same for all
    Nss = len(weights)
    # we expect mdtraj has dims (nmol*natoms/mol)
    n_mol = mdtraj.n_residues
    n_atms_p_mol = mdtraj.n_atoms / mdtraj.n_residues
    ...
    part_ind = part_ind.tolist()
    # define the dimensions of the feature
    feature_vec = np.zeros(shape=(mdtraj.n_frames,Nss,len(part_ind)),dtype=np.float32)
    # get some grid info
    N_grids = r.shape[0]
    dr = r[1] - r[0]
    for fr in range(mdtraj.n_frames):
        ...
        if ( np.any(np.isnan(dist)) ):
            np.save('dist_err', dist)
            raise ValueError('nan found in the dist vec for fr %d, see dist_err.npy' % (fr))
        for ss in range(Nss):
            # select indices of pairs within the cutoff
            coord = np.where(dist < rcut[ss])[1]
            # get the grids corresponding to these distances
            grids = ((dist[0,coord] - r[0])/dr).astype(int)
            # and the corresponding weights
            weight_grids = np.array(weights[ss])[grids]
            # store the identifier of each pair separately (i.e., each part has it's own features)
            set1 = pairs[coord][:,0]
            set2 = pairs[coord][:,1]
            # now sum up the weights for each particle
            ...
            # finally, store the feature
            ind = [part_ind.index(i) for i in ugroup]
            feature_vec[fr,ss,ind] = sums
#        if ( fr % mdtraj.n_frames/5 == 0):
#            print 'On frame '+str(fr)+' of '+str(mdtraj.n_frames)+' for current traj...'
    return feature_vec

# loop over different solvation shell selections within the md loop
def calc_wghtd_CN_multss_new( mdtraj, (sel, pairs, rcut, r, weights) ):
    # get the number of solvation shells, assume for now sel, pairs and r is the same for all
    Nss = len(weights)
    # we expect mdtraj has dims (nmol*natoms/mol)
    n_mol = mdtraj.n_residues
    ...
    for fr in range(mdtraj.n_frames):
        ...
        dist = md.compute_distances(mdtraj[fr], pairs, periodic=True, opt=True)
        # select indices of pairs within the cutoff
        coord = np.where(dist < rcut[ss])[1] # just do this once for the longest cut-off
        # get the grids corresponding to these distances
        grids = ((dist[0,coord] - r[0])/dr).astype(int)
        weight_grids = []
        # store the identifier of each pair separately (i.e., each part has it's own features)
        ...
        for ss in range(Nss):
            # and the corresponding weights
            weight_grids = np.array(weights[ss])[grids] # I don't know how to do this with a multi-D array
            for group in ugroup1:
                sums1.append(weight_grids[set1 == group].sum())
            for group in ugroup2:
                sums2.append(weight_grids[set2 == group].sum())
