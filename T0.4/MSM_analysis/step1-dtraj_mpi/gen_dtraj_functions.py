...
def feat_CN_per_mol_reonly( X, n_frames_tot, n_frames_p_traj, n_traj, n_feat_p_mol, n_mol ):
    Xre = []
    for i in range(n_traj):
        for j in range(n_mol):
            Xre.append(X[i*n_frames_p_traj:(i+1)*n_frames_p_traj,:,j])
...
def calc_chunkwise_noavg_mpi( func, traj_list, top_file, n_proc, rank, chunk_size=1, dim=1, stride=1, skip=0 ):
# This function computes some observable from an md traj in trunks, as to not use too much memory
# It assumes that the passed in function has no additional input (e.g., use a lambda function)
# and that the output is to be (non-weighted) averaged over chuncks
    count = 0
    ...
    func_ret = []
    for i in range(len(traj_list)):
        for index, chunk in enumerate(md.iterload(traj_list[i], chunk=chunk_size, top=top_file, stride=stride, skip=skip)):
            if ( index%n_proc != rank ): 
                continue    
            print 'proc '+str(rank)+' working on chunk '+str(index+1)+' from traj '+str(i+1)+':'+str(chunk)
            func_ret_tmp = func(chunk)
            frames_p_chunk.append(chunk.n_frames)
            if (count==0):
                func_ret = np.array(func_ret_tmp)
            else:
                if (dim==1):
                    ...
                    for j in range(dim):
                        func_ret[j] = np.concatenate((func_ret[j],np.array(func_ret_tmp)[j]),axis=0)
            count += 1
        print 'proc '+str(rank)+' done with traj # '+str(i+1)

    return func_ret, frames_p_chunk
