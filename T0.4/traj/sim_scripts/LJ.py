...
L_LATT_PARAM_SIGMA  = [60,10,10]	# box edge lengths in units of latt. parameter		[Lx will be rescaled]
NPART_TOT           = 5000		# total number of particles in the system
NUMDENS             = 0.5400		# number density
MOLFRAC             = 0.2000		# molar fraction solute
...
SIGMASW             = 0.8000		# sigma parameter solute-solvent 			["mix" or value]
EPSILONSW           = 1.5000		# epsilon parameter solute-solvent 			["mix" or value]
...
SIGMASS             = 0.8800		# sigma parameter solute-solute
EPSILONSS           = 0.5000		# epsilon parameter solute-solute
...
TEMPERATURE         = 0.2000		# temperature						
PRESSURE            = 0.0000		# pressure Pxx						["auto" or value, only need for constant-P ensembles]
PISTON_MASS_EQ      = 1.0000		# effective piston mass for barostat (eq)		[only need for constant-P ensembles]
PISTON_MASS_SIM     = 1.0000		# effective piston mass for barostat (sim)              [only need for constant-P ensembles]
...
CMUMD_MODE          = "off"		# switch to apply force in membrane			["solute", "solvent", "both" or "off"]
...
TOTAL_SIM_STEPS     = 1000000		# total number of simulation steps			
...
CONFIG_EVERY_SIM    = 100		# md steps between configuration outputs (simulation)
RESTART_EVERY       = 5000000		# md steps between restart file writings
...
while (RESTART_EVERY%EQUIL_ISTEPS!=0) or (RESTART_EVERY%SIM_ISTEPS!=0):
  RESTART_EVERY      += 1
RESTART_EVERY_EQSTEP  = int(RESTART_EVERY/EQUIL_ISTEPS)
